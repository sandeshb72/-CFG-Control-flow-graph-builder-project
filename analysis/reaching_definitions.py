...
def _def_id(var, block_id, instr_idx):
    """Creates a unique identifier for a definition: 'variable@block_id@instr_idx'."""
...
def reaching_definitions(graph, blocks, entry_id):
    """
    Performs Reaching Definitions analysis using the iterative worklist (Fixpoint) algorithm.
    Sets 'rd_in' and 'rd_out' for each block.
    """
...
def find_uninitialized_uses(blocks):
    """
    Identifies variables that are used in a block but might not have been initialized.
    A variable is uninitialized if it is used before being defined, AND no definition reaches it from IN[B].
    """
    issues = []
    for blk in blocks.values():
        defined_before = set()
...
def get_reaching_defs_at(block_id, blocks):
    """Helper to return a more readable map of reaching definitions: { 'var': {source_block_ids} }"""
    blk = blocks[block_id]
    result = {}
    for d in blk.rd_in:
        parts = d.rsplit('@', 2)
